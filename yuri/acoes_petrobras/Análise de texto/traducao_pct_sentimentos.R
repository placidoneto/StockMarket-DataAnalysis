...
get_compare_lexical_table <- function(){
      ...
      tabela_sentimento_estr <- read.table("Análise de texto/subjclueslen1-HLTEMNLP05.tff",header = F)
      
      col_sentimento <- sub("priorpolarity=", x = tabela_sentimento_estr$V6, replacement = "")
      
      # write.csv(col_palavras, "texto")
      
      col_palavras_traduzida <- read.csv("Análise de texto/traduzido.txt", stringsAsFactors = F)
